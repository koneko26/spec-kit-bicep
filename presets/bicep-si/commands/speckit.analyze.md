...
- **カバレッジ・マトリクス検査**（必須）: 各 **非機能要件(NF-xxx / 9区分)** と各 **WAF 5本柱** が、
  要件 → 基本設計 → 詳細設計 → tasks → 単体テスト観点まで**追跡可能か**を表で検査する。未カバーの
  柱・カテゴリ（例: 性能の検証観点が無い、バックアップが設計に落ちていない）を不整合として列挙する。

  | NF / WAF柱 | 要件 | 基本設計 | 詳細設計 | tasks | test観点 | 状態 |
  |------------|:--:|:--:|:--:|:--:|:--:|:--:|
  | NF-SEC / セキュリティ | | | | | | |
  | NF-REL / 信頼性 | | | | | | |
  | ... | | | | | | |
